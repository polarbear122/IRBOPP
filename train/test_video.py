...
if __name__ == "__main__":
    start_at = time.time()
    log.logger.info("多帧测试开始-------------------------------------------")
    all_norm_pose, all_label, all_video_length_list = read_data.read_data_stream_test()
    get_data_at = time.time()
    log.logger.info("测试联合分类器, 测试data大小(%d,%d),%d" %
                    (all_norm_pose.shape[0], all_norm_pose.shape[1], all_label.shape[0]))

    forest_model = load_model("trained_model/video/Forest_image.model")
    ...
    np.savetxt("../train/trained_model/video/Forest_image_y_pred.csv", y_pred, delimiter=',')
